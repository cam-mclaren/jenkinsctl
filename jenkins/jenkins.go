...
type Config struct {
	...
	SSLVerification bool   `mapstructure:"SSLVerification"`
	ConfigPath      string
...
func (j *Jenkins) Init(config Config) error {
	...
	j.SSLVerification = config.SSLVerification

	// Configure a client with the desired SSL settings
	client := &http.Client{
		Transport: &http.Transport{
			TLSClientConfig: &tls.Config{
				InsecureSkipVerify: !j.SSLVerification,
			},
		},
	}
